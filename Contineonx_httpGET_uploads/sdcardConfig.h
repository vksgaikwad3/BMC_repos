...
/*** Checking if File Exist or not *******************/
  ...
  if (SD.exists(filename)) {
    ...
    if(myFile.size() <= 0)    //check Size of file is zero or not
    {
     ...
     myFile.println("Date,Time.,DeviceID,BatteryTemp(oC),MilkTemp(oC),AuxillaryTemp(oC),BatteryVoltage(VDC),ACVoltage(ACV),CompresorCurrent(A),PumpCurrent(A),ChargingPumpRelay,CondensorRelay,CompressorRelay,InverterRelay,AgitatorRelay,TankRelay,ShivaRelay,Disch.PumpRelay,CompressorRunHours, \tDeviceID,LineVolts[VAC],LineCurrent[A],DeviceVolts[VDC],DeviceCurrent[A],Power[W],PowerConsump[kWh],DeviceRunHr[Hrs],PowerAvailable[W]");
