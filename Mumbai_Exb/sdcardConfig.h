...
boolean writetoSDCard (String filename,byte mode,String logTime,int deviceID1,float battery_Temp,float milk_Temp,float auxillary_Temp,float battery_Volt,float ac_Volt,float compressor_Current,float pump_Current,
                       ...
                       int deviceID2,float em2_lineVolts,float em2_lineCurrent,float em2_powerFactor,float em2_power, float em2_deviceRunHr,float em2_powerAvailableTime,float em2_VA_Avg);
...
/*** Checking if File Exist or not *******************/
  ...
  if (SD.exists(filename)) {
    ...
    if(myFile.size() <= 0)    //check Size of file is zero or not
    {
     ...
     myFile.println("Date,Time,DeviceID,BatteryTemp(oC),MilkTemp(oC),AuxillaryTemp(oC),BatteryVoltage(VDC),ACVoltage(ACV),CompresorCurrent(A),PumpCurrent(A),ChargingPumpRelay,CondensorRelay,CompressorRelay,InverterRelay,AgitatorRelay,TankRelay,ShivaRelay,Disch.PumpRelay,CompressorRunHours, \tDeviceID,LineVolts[VAC],LineCurrent[A],Power[W],PowerConsump[kWh],DeviceRunHr[Hrs],PowerAvailable[W],VA_Average");
...
//    data +=  "\t\t";
    ...
    data += em2_lineVolts;
    data += ",";
    data += em2_lineCurrent;
    data += ",";
    data += em2_powerFactor;
    data += ",";
    data += em2_power;
    data += ",";
    data += em2_deviceRunHr;
