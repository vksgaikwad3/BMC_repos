...
/******************************* Auto SIM Detection ************************************/
// Author : Tanamayee Mete
// Date : 07/04/2017  


char* sim900_GPRS::Check_Operator() 
{
  
  String arr[] = {"AirTel", "IDEA", "Vodafone", "TATA DOCOMO"};
  String firstVal, c;
  ...
  if(Serial1.available() )
  {
   ...
   // Serial.print("c: ");
   // Serial.println(c);
//    String APN = Check_Operator();
//    Serial.print("APN is: ");
//    Serial.println(APN); 
//  
  
    int commaIndex = c.indexOf('"');
    String input = c.substring(commaIndex + 1);
   ...
   // Serial.println(input);
    ...
    for (int j = 0 ; j < 4 ; j++) {
      if (firstVal == arr[0]) {
        Serial.println("This is Airtel sim card");
        //return "\"airtelgprs.com\"";
        return "airtelgprs.com";
        break;
      }
      else if (firstVal == arr[1]) {
        Serial.println("This is Idea sim card");
        //return "\"imis/internet\"";
        return "imis/internet";
        break;
      }
      else if (firstVal == arr[2]) {
        Serial.println("This is Vodafone sim card");
        //return "\"www\"";
        return "www";
        break;
      }
      else if (firstVal == arr[3]) {
        Serial.println("This is TATA DOCOMO sim card");
        //return "\"tata\"";
        return "data";
        break;
      }
    ...
    do{
        ...
        if(Serial1.available() != 0){    
            ...
            else if (strstr(response, expected_answer2) != NULL)    
            {
             ...
             else if(strstr(response, "#RESET") != NULL || strstr(response, "RING") != NULL || strstr(response,"+PDP: DEACT"))    
            {
                Serial.println("#RESET Found in a SMS or +PDP: DEACT");
...
void sim900_GPRS::SendMessage()
{
  ...
  delay(1000);
  Serial1.println(msg);// The SMS text you want to send
  delay(100);
//  Serial1.write(0x1A);
//  Serial1.write(0x0D);
//  Serial1.write(0x0A);
...
void sim900_GPRS::verifyData(int conreq,int failreq,int sucessreq,float battemp,float milktemp)
{
  String msg = "DATA:";
  msg += "\n";
  msg += "CONREQ:";
...
void sim900_GPRS::GPRSFailedSMS()
{
  ...
  Serial1.println("AT+CMGS=\"+918983537961\"\r"); // Replace x with mobile number
  delay(1000);
  Serial1.println("GPRS Connection Failed, Retrying to connect, at Viz1,Heritage Dairy Maharashtra");// The SMS text you want to send
  delay(100);
//  Serial1.write(0x1A);
//  Serial1.write(0x0D);
//  Serial1.write(0x0A);
