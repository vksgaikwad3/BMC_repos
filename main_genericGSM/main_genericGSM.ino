...
volatile uint8_t tot_overflow =0;  // global variable to count the number of overflows
...
//void timer1_init()              // initialize timer, interrupt and variable
//{
//  TCCR1B |= (1 << CS12);        // set up timer with prescaler = 1024
//  TCCR1B |= (1 << CS10);
...
//
//
//ISR(TIMER1_OVF_vect)            // called whenever TCNT1 overflows
//{
//  tot_overflow++;
//  if (tot_overflow >=14)       // NOTE: '>=' used instead of '==' // For 30 min = 429 ; 15 min = 214 ; 10 min = 143 ; 1min = 14 
//  {
//    //PORTC ^= (1 << 0);          // toggles the led no timer reset required here as the timer is reset every time it overflows
//    //digitalWrite(2,HIGH);
...
//  if (currentMillis - previousMillis >= 60000) {     // 15 Mins
//    previousMillis = currentMillis;
//    resetFunc();
////    Serial.println("***************************");
////    Serial.print("In one minute");
////    Serial.print("\t ");
...
{
    tot_overflow++;
     if(tot_overflow >= 12)         // cnt 1 Min = 12 , 5 min = 60
     {
      // Toggle LED
      Serial.println("\t\n 1 MIns ");
      //digitalWrite( 2, digitalRead( 2 ) ^ 1 );
